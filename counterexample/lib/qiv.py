"""Exact rational interval / box arithmetic. Track B.

Every endpoint is a fractions.Fraction. No floating point anywhere; the only
integer primitive beyond ring ops is math.isqrt (exact). Boxes are closed
rectangles [re.lo, re.hi] x [im.lo, im.hi] guaranteed to contain the value.

enclose(expr, eps) walks a sympy expression tree and returns a certified box of
half-width <= eps per component, or raises EncloseError when it meets a node it
cannot certify (caller escalates -- never guesses).
"""
